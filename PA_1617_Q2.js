...
function all_different(nums) {
    ...
    const filtered = filter(x => x === head(nums), tail(nums));
    
    return is_null(filtered) && all_different(tail(nums));
...
function is_valid_toto_set(nums, n, min, max) {
    ...
    const findmin = filter(x => x < min, nums);
    const findmax = filter(x => x > max, nums);
    
    return all_different(nums) && is_null(findmin) && is_null(findmax);
}
...
function num_of_matches(numsA, numsB) {
    let count = 0;
    let n = map(x => filter(y => x === y, numsB) , numsA);
...
function check_winning_group(bet_nums, draw_nums, extra_num) {
    ...
    !is_null(filter(x => x === extra_num ,bet_nums))) {
        return 2;
    } else if (num_of_matches(bet_nums, draw_nums) === length(draw_nums) - 1) {
        return 3;
