...
function oxoguanine_repair(xs) {
    ...
    return pair(head(xs) === "8"? "G" : head(xs), oxoguanine_repair(tail(xs)));
...
function find_gene_start(xs) {
    function find(xs) {
        ...
        if (head(xs) === "A" && head(tail(xs)) === "T" && head(tail(tail(xs))) === "G") {
...
function find_gene_end(xs) {
    ...
    function helper(xs, result) {
        if (length(xs) < 3) {
            return append(result, xs);
        }
        if (head(xs) === "T") {
            if (head(tail(xs)) === "A" && head(tail(tail(xs))) === "G") {
                return result;
            }
            if (head(tail(xs)) === "A" && head(tail(tail(xs))) === "A") {
                return result;
            }
            if (head(tail(xs)) === "G" && head(tail(tail(xs))) === "A") {
                return result;
            } else {
                return helper(tail(xs), append(result, list(head(xs))));
            }
        } else {
            return helper(tail(xs), append(result, list(head(xs))));
        }
...
function all_genes(xs) {
    function helper(xs) {
        ...
        display(end);
        return append(end, helper(head(start)));
    }
